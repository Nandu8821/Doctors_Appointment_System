...
const Appointment = () => {
  ...
  const getAvailableSlots = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      while (currentDate < endTime) {
        ...
        const isSlotAvailabe = docInfo.slots_booked[slotDate] && docInfo.slots_booked[slotDate].includes(slotTime) ? false : true
  ...
  const bookAppointment = async () => {
    ...
    try {
      ...
      let month = date.getMonth() + 1; // Fix: month is 0-indexed
      ...
      const { data } = await axios.post(
        ...
        {
          headers: { token }, // or use Authorization: `Bearer ${token}`
        }
      );
  ...
  <div className="w-full sm:w-auto flex justify-center">
    <img
      className="bg-primary w-full max-w-[300px] sm:max-w-72 rounded-lg object-cover"
      src={docInfo.image}
      alt="Doctor"
    />
  </div>
  ...
  {/* Doctor Info */}
  <div className="flex-1 border border-gray-300 rounded-lg p-6 sm:p-8 bg-white mt-[-60px] sm:mt-0">
    ...
    <p className="text-gray-500 font-medium mt-4">
      Appointment fee:{" "}
      <span className="text-gray-700 font-semibold">${docInfo.fees}</span>
    </p>
  </div>
</div>
              ...
              docSlots[slotIndex].map((item, index) => (
                <p
                  ...
                  className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${
                    item.time === slotTime
                      ? "bg-primary text-white"
                      : "border border-gray-400 border-gray-300"
                  }`}
                >
